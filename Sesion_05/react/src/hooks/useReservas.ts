...
interface UseReservasResult {
  ...
  reprogramar: (id: string, fechaInicio: string, fechaFin: string) => Promise<void>;
}
...
export function useReservas(): UseReservasResult {
  ...
  const reprogramar = useCallback(async (id: string, fechaInicio: string, fechaFin: string) => {
    setReprogramandoId(id);
    try {
      await reprogramarReserva(id, fechaInicio, fechaFin);
      setReservas((actuales) =>
        actuales.map((reserva) =>
          reserva.id === id ? { ...reserva, fechaInicio, fechaFin, estado: 'reprogramada' } : reserva,
        ),
      );
    } finally {
