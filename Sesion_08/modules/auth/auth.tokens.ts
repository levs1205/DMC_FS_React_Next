...
const ACCES_TOKEN_TYPE = "access";
const REFRESH_TOKEN_TYPE = "refresh";

const encoder = new TextEncoder();
const accessKey = encoder.encode(authConfig.accessTokenSecret);
const refreshKey = encoder.encode(authConfig.refresTokenSecret);
...
export async function verifyAccessToken(
  ...
  try {
    ...
    if (payload.tokenType !== ACCES_TOKEN_TYPE || !Number.isInteger(id)) {
      return null;
    }
